...
<form method="post" class="ccm-dashboard-content-form" action="<?php echo $view->action('update_sitename')?>">
	...
	<div class="form-group">
		<label for="DESC" class="launch-tooltip control-label" data-placement="right" title="デフォルトのメタディスクリプションを設定して下さい">デフォルト<?php echo t('Meta Description'); ?></label>
		<?php echo $form->text('MDESC', $mdesc, array('class' => 'span4'))?>
	</div>

	<div class="form-group">
		<label for="KEY" class="launch-tooltip control-label" data-placement="right" title="デフォルトのメタキーワードを設定して下さい">デフォルトMetaタグキーワード</label>
		<?php echo $form->text('MKEYWORD', $mkeyword, array('class' => 'span4'))?>
	</div>
	</fieldset>
